...
export function draw_wait(
  ...
  ctx.font = font_text;
  ctx.fillText("Waiting for Player", 175, height / 2);
}

export function draw_already(
  color: boolean,
  ctx: CanvasRenderingContext2D,
  game: GameState,
  colorObject: string
...
export function draw_all(
  ...
  if (game.state === PLAY) draw_game(color, ctx, game, colorObject);
  else if (game.state === OVER) draw_end(color, ctx, game, colorObject);
