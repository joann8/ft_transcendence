...
export default function OtherUser() {
  ...
  const handleBlocking = async (status) => {
    if (status === 4) await removeRelation(otherUserData.id_pseudo);
    else await updateRelation(otherUserData.id_pseudo, 4, 5);
    setUpdate(!update);
  };
  ...
  function FriendButton({ status }) {
    ...
    //ADD or WAITING or ACCEPT
      ...
      if (status === 2) addButton = "Accept";
      else addButton = status === 0 ? "Add" : "Waiting";
      console.log("ici?");
      return (
        <Button
          variant="contained"
  ...
  function ButtonBlock({ status }) {
    ...
    return (
      <Fragment>
        {otherUserData.achievement1 ? (
          <Chip
            sx={{ margin: "2px" }}
            icon={<StarsIcon />}
            label="Win with Max Score (3-0)"
            color="success"
          />
        ) : (
          <Chip
        ...
        {otherUserData.id !== context.user.id && (
          <FriendButton status={status} />
        )}
      </Fragment>
    );
  }
